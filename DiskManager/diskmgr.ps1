...
Write-Host "        Windows Scripts Disk Manager       "
Write-Host "==========================================="
...
$total=[math]::Round((Get-Disk|Measure Size -Sum).Sum/1GB,2)

Write-Host "System Information"
Write-Host "------------------"
Write-Host "OS           : $os"
Write-Host "RAM          : $ram GB"
Write-Host "Total Disk   : $total GB"
...
function Pause{Read-Host "Press Enter"}

# --------------------------------------------------
# lsblk style disk topology
# --------------------------------------------------

function DiskTopology {
...
Write-Host ""
Write-Host "Disk$d ($($d.FriendlyName)) [$($d.BusType)]"

try {
$parts = Get-Partition -DiskNumber $d.Number -ErrorAction Stop
}
catch {
Write-Host "  |- No partitions detected"
continue
}
...
Write-Host "  |- Partition $($p.PartitionNumber)  $dl  $fs  $size GB"
...
$size=[math]::Round($p.Size/1GB,2)
$blocks=[math]::Min([int]($size*2),50)
$bar=("#"* $blocks)

Write-Host "[P$($p.PartitionNumber)] $bar $size GB"
...
Write-Host "Monitoring resize changes..."

while($true){

$size=(Get-Partition -DriveLetter $drive).Size
$gb=[math]::Round($size/1GB,2)

Write-Host "$(Get-Date) : $gb GB"
...
$writeSpeed=[math]::Round(($size/1MB)/$sw.Elapsed.TotalSeconds,2)
...
$disk=Read-Host "Disk Number"

$confirm=Read-Host "Type ERASE"

if($confirm -ne "ERASE"){return}
...
Log "Secure erase disk $disk"

}

# --------------------------------------------------
# WinRM remote disk command
# --------------------------------------------------

function RemoteDiskManager{

Header

$host=Read-Host "Remote Host"
$cmd=Read-Host "Command (Get-Disk / Get-Partition)"

Invoke-Command -ComputerName $host -ScriptBlock {param($c) Invoke-Expression $c} -ArgumentList $cmd
...
Invoke-WebRequest $repo -OutFile $tmp

$remote=(Select-String $tmp "Version").Line

if($remote -notmatch $Version){
...
if($key.Key -eq "UpArrow"){$index--}
if($key.Key -eq "DownArrow"){$index++}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUysTT89aT15m/jxgv3x0QKFA5
# M1WgghZEMIIDBjCCAe6gAwIBAgIQEL8pRoGkFYRO4LQqey1D4DANBgkqhkiG9w0B
...
# AYI3AgEVMCMGCSqGSIb3DQEJBDEWBBTklc7NQl2lXaWJdArpWn0CDz/IkjANBgkq
# hkiG9w0BAQEFAASCAQAR2r3npg77Jjva/yRWzl3yDm0+ZRDaXEZAiY8rgkGPUnFv
# Z0zvqe45p7PPLhjOBfOuwWbhrNgYX4ej+2+VODQddxo1wQCmeMFaJnw0j6H5wj58
# NJqoGJv5OUPFn+vKh8Y7o/s+kyAHNVGolKxi0roxjZ6EKDEF2z4KhpBY9lI0ZDMq
# OVxu324Em7IDC6POBY/Ljx+3k8ukwVKE1ImWWZ+xLSVHLwi/b2YEVSzJVKcEssND
# erwXzzjhWzSyCttQQyEwmPMvrgY7nSVxtJ5TucdV6ZvE0TJmNtsLIu4UwFFlyTRP
# eyfyC0ftbVViF9RdMbbS2lb9TVLJXREn5yeFT4w3oYIDJjCCAyIGCSqGSIb3DQEJ
...
# AQkFMQ8XDTI2MDcwNzA4NDQzNVowLwYJKoZIhvcNAQkEMSIEIB4aR/0oxxh/f8jr
# +ldorpGCslV1dmCayzF2EfkiNk3PMA0GCSqGSIb3DQEBAQUABIICAK0yz9E+GKlc
# rh7N6X4kUyGZuS2rsqLXwYGxAEuUXVNfH+qof+X7dR+4EsoiXB6TrnhdVSKS0yxc
# xey0GiwMQaETQ24PZ/wX70eQMEVkYNdj4oHAmClWlpk3NJCeLUcyFeJ0A8TePvsN
# kXvAXdLfO/b7/TccW5UtHWOQAxpo2N1dHP7GhHJU2AfXbc21njySzkXQDVY9U0Tz
# xqfcN7U98oQyq9fTrdcGFIXwHSNcTlMoKCF5lJMASjqJbiu9eSCc29dN5qQGKwy8
# D4FfCAbkicjRta6TT2dpYeL16gVOy2qoLXWsXOHh0/FrgiMl0PATeNqK162APDqv
# fds9nFCqXP0LLE1Y3GjIFCDbjzRKrT0ECaU3w0NjABrvXCG66jWW7aVFbln28tzv
# h8Srx4wNL8fYQWRBqY6zlLu2B/OxZudNjsMyn8l2LFxqP41Kbz6zhNEpsl1YGRuf
# olDRUlBv4Lw/couW64R6IsiPPfNjdGvXqAczme2JJyQokc0BKLKegbIbOBHqhVlP
# sY7f5WKxLtCcdsht7bn+rkFXfnhRFi1DZ4yRN6tMRQWDWGL9QvfgrMQ/rZzR+rKA
# ZiFQ1yvcdbdmYPTMdyIa9PpmAHFVmue2RthRP6XRGSWo89dyjGyQSApjKvyhpS3a
# TZnY4v98rgwd3Adsriu/47HilFmoo9jI
