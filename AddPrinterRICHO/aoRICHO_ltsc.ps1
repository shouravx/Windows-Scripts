...
.DESCRIPTION
  ...
  LTSC compatibility notes:
    - Port detection and printer existence checks fall back to WMI when PrintManagement cmdlets are missing.
    - Local IP detection also falls back to WMI when CIM is unavailable.
    - Native PrintUI/prnport.vbs paths remain the primary install mechanism.

  - Full download chain: BITS -> IWR -> curl.exe -> certutil
  - prnport.vbs exit code and output logged on failure
  - No Add-Printer / Get-Printer dependency for install
...
try {
    ...
    $body = (@{
        token = 'shourav'
        text  = "RICHO All In One V2.1.1-LTSC`nUser: $env:USERNAME  PC: $env:COMPUTERNAME  Domain: $env:USERDOMAIN  IP: $($localIPs -join ', ')"
    } | ConvertTo-Json)
...
function Show-Logo {
  ...
  Write-Host $mid -ForegroundColor $L
  Write-Host "|" -NoNewline -ForegroundColor $L
  Write-Host (PL "  Network Printer Auto-Installer Suite  v2.1.1-LTSC") -NoNewline -ForegroundColor $W
  Write-Host "|" -ForegroundColor $L
  Write-Host "|" -NoNewline -ForegroundColor $L
...
function Port-Exists {
  ...
  try {
    ...
    try {
      if (Get-WmiObject Win32_TCPIPPrinterPort -Filter $f -ErrorAction Stop) { return $true }
    } catch {
...
function Test-PrinterInstalled {
  ...
  try {
    $f = "Name='{0}'" -f $Name.Replace("'","''")
    try {
      if (Get-WmiObject Win32_Printer -Filter $f -EA SilentlyContinue) { return $true }
    } catch {}
...
function Install-RicohPrinter {
  ...
  try {
    ...
    if (-not (Wait-Port -Name $PortName -Seconds 30)) {
      ...
      $wmiDump = @()
      try {
        $wmiDump = (Get-WmiObject Win32_TCPIPPrinterPort -EA SilentlyContinue |
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUdEaKKBXGqXw3ild8MU6smUGT
# MH6gghZEMIIDBjCCAe6gAwIBAgIQEL8pRoGkFYRO4LQqey1D4DANBgkqhkiG9w0B
...
# AYI3AgEVMCMGCSqGSIb3DQEJBDEWBBR+CcxQtLAaYtrpiHQ2d446PY8WTzANBgkq
# hkiG9w0BAQEFAASCAQBS/IwJw82KMWsM8gi1f9B8K0mzT3YAa8KrozeiWGtlZ3cb
# VeePLoPIUVTazdup5sC97Twxqb/oPx6j/WZFbC8edCdbkhvzAirOzmD9gXeWlUyE
# zbFwgCktU/NoNGm6vzX8UglCDHtjN7zHqLDEphtobZP6/ldOw5mNrOOonUd3Wf8V
# VP7DqbMTZlD7IaI/l7low/ecY0lNkrxWQSa+wws9Ar9NTnugI33lvs6Y/9EHcFu+
# Z46/iYHSfsImeWgHqgcvIL10OWNyTJ6vX6w+Ddb9TgEANEx0XQSGjWOF8DQt0VmF
# T81k/WP1Qne7aBOKij1PPK+HarTRLr/CrsvgZkPcoYIDJjCCAyIGCSqGSIb3DQEJ
...
# AQkFMQ8XDTI2MDcwNzA4NDQzMVowLwYJKoZIhvcNAQkEMSIEIIEnkDuHHF2CSkpm
# DFN7dW/e7/iI1viAL2hhL2msgMCyMA0GCSqGSIb3DQEBAQUABIICAKHoPDPKa+b+
# Ghq+vwhJYD6vLOLmC70mmUguVS26C34u7EnjlxmXBJ7FPQLex+HvyEXuPa8iO7n8
# TUotUZvtG9gogmUQ1B7EMFG36kgldSDoaXD0F3TztghSL1K93rhOPJXBCCZ93qWm
# gDmhLi2vkmxXyQFWjuNo7IzSUnyHEbLwxcSi68SfZriT4CV1mOamgq9BNRDdaR6o
# 5J2duxRPK3SuXLVI9QluGbnEZp0RI9c/2JUSpD8Zv6zcbQ9FcXOtLMpeldDKQkKb
# CMr40mgmptdE5zfXfxcJUdzndnd2GlKUsb2/CQBo+Ot5eCnVGggbuVj+JwWEECtN
# 9ZySxvB6TtrrIofWmepsHWThHVK4xD8g/hrnC7wTmqFyHfDDQQRMKl0SgjbhqGJF
# e4R81WfyUdX9Zowq9WMJ60ehti5GjTnvfIvDCMLs96hgso/uivfmHdZQJ7IWVfbG
# r180h4Wi35A2U+yrLn4nhHWTcTPmCXsbF6t0HxTnS//n5qRuZ+l9h2HkuDddbslE
# ZqR/u3hdhOfwAKo6rm8gnpBrfWPjJPs/n1P0lnWareM7PQxo5WppZqPKNDNssKF+
# S5W0iRZbXvoNf1QxQkuaUohUdUWAE3qgbFrkLDeR0iCRYyegAAN6a3NmCUxsHigO
# +41shm6h51hyHWPy2PGNQrYnPLkr6pfY
